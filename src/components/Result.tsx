...
<img
 ...
 <p className="text-3xl font-semibold text-center mb-6 text-white bg-gradient-to-r from-purple-600 to-blue-600 p-4 rounded-lg shadow-lg max-md:text-xl">
    {t('game-over')}
  </p>

  <p className="text-2xl text-white mb-4 font-medium">
    {t('your-score')} <span className={`font-bold ${score >= 7 ? 'text-green-400' : 'text-red-400'}`}>{score}</span>
  </p>

  <p
    data-testid="outcome"
    className={`text-2xl font-bold mb-6 ${score >= 7 ? 'text-green-500' : 'text-red-500'} transition-colors duration-300`}
  >
